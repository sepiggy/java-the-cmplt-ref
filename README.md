...
1. 类和包都是封装以及包含变量与方法的名称空间和作用域的手段。

2. 包是类和其他子包的容器；类是数据和代码的容器，类是Java中最小的抽象单元。

3. 因为类和包相互影响，所以Java为类成员提供了四种不同类别的可见性：
...
4. 三种访问修饰符
   `private`、`public`和`protected`：为生成上述四种类别所需要的多级访问提供了各种各样的方法。

5. 类的成员的访问级别（根据四种类别和三种修饰符进行划分）

   |                   | private | 无访问修饰符 | protected | public |
   | :---------------: | :-----: | :----: | :-------: | :----: |
   |     在同一个类中可见      |    是    |   是    |     是     |   是    |
   | 对相同包中的类（子类或非子类）可见 |    否    |   是    |     是     |   是    |
   |    对不同包中的子类可见     |    否    |   否    |     是     |   是    |
   |    对不同包中的非子类可见    |    否    |   否    |     否     |   是    |

   - 所有声明为`public`的成员可以在任何地方进行访问。
   - 所有声明为`private`的成员在类的外部都不可见。
...
1. 在Java源文件中，import语句紧跟package语句（如果有的话）之后，并且在所有类定义之前。
...
2. 如果没有提供访问修饰符，将采用默认访问级别，并且只有声明接口的包中的其他成员才能访问接口。
   如果将接口声明为public，那么所有代码都可以使用接口。

3. 接口中声明的变量，被隐式地标识为final，这意味着实现接口的类不能修改它们。

4. 接口中声明的变量，被隐式地标识为static，这意味着实现接口的类必须初始化它们。

5. 接口中所有的方法和变量都隐式地声明为public。

#### 9.4.2  实现接口

1. 实现接口方法时，必须将其声明为public。
2. 使用接口引用访问实现（类似于使用超类引用访问子类对象）。
3. 接口可以实现多态。
4. 如果类包含了一个接口，但是没有实现该接口定义的全部方法，那么必须将类声明为`abstract`。

#### 9.4.3 嵌套接口

1. 可以将接口声明为某个类或另外一个接口的成员，这种接口被称为`成员接口`或`嵌套接口`。

2. 嵌套接口可以被声明为public、private或protected；而顶级接口要么被声明为public，要么使用默认访问级别。

3. 当在封装范围之外使用嵌套接口时，必须使用包含嵌套接口的类或接口的名称进行限定。
...
1. 可以使用接口将共享的常量导入到多个类中，具体方法是简单地声明包含变量的接口，并将变量初始化为期望的值。
2. 当类实现这种接口时，所有这些变量在作用域内都会被作为常量。
3. 如果接口不包含方法，那么实现这种接口的所有类实际上没有实现任何内容。就好比是将常量作为final变量导入到类名称空间中。

#### 9.4.6 接口可以扩展

1. 接口可以通过关键字`extends`继承另一个接口，语法和继承类相同。
2. 如果类实现的接口继承自另外一个接口，那么类必须实现在接口继承链中定义的所有方法。即：
   `实现接口的所有类必须实现接口定义的所有方法，包括从其他接口继承过来的方法。`

### 9.5 默认接口方法

1. JDK8为接口添加了一种新功能，叫做`默认方法`。默认方法允许为接口方法定义默认实现。

2. 添加默认方法并没有改变接口的关键特征：`不能维护状态信息`。eg:接口仍然不能有实例变量。

3. 为接口定义默认方法，类似于为类定义方法。主要区别在于，默认方法的声明前面带有关键字`default`。eg:
...
4. 多级继承问题的处理规则
   ...
   - 类实现的优先级高于接口的默认实现。
   - 当类实现的两个接口提供了相同的默认方法，但是类没有重写该方法时，会发生错误。
   - 如果一个接口继承了另一个接口，并且两个接口定义了相同的默认方法，那么继承接口的版本具有更高优先级。

5. 通过使用super的一种新的形式，可以显式引用被继承接口中的默认实现，如下：
...
1. JDK8为接口添加了另一项新功能：`定义一个或更多个静态方法`。

2. 在调用静态方法时，不需要实现接口，也不需要接口的实例。类似于类中的静态方法。其一般形式如下：

   ```java
   InterfaceName.staticMethodName
   ```

3. 实现接口的类或子接口不会集成接口中的静态方法。
...
1. Java异常是用来描述在一段代码中发生的异常情况的`对象`。当出现引起异常的情况时，就会创建用来表示异常的对象，并在引起错误的`方法`中抛出异常对象。
2. 方法可以选择自己处理异常，也可以继续传递异常。
3. 异常可以由Java运行时系统生成，也可以通过代码手动生成。
4. Java异常处理的5个关键字：`try`、`catch`、`throw`、`throws`以及`finally`。

### 10.2 异常类型

### 10.3 未捕获的异常

1. 没有被程序捕获的所有异常，最终都将由默认处理程序进行处理。

2. 默认处理程序会显示一个描述异常的字符串，输出异常发生点的堆栈踪迹并终止程序。eg
   ...
   注意类名（Exc0）、方法名（main）、文件名（Exc0.java）以及行号（4），是如何都被包含到这个简单的堆栈踪迹中的。

### 10.4 使用try和catch

1. 使用try和catch可以自己处理异常，这有两个优点：
   - 允许修复错误
   - 阻止程序自动终止
2. 为了防止并处理运行时错误，在try代码块中封装希望监视的代码。紧随try代码块之后，提供一条catch子句，指定希望捕获的异常类型。
3. try及其catch语句构成了一个单元。catch子句的作用域被限制在由之前try语句指定的那些语句内。
4. 由try保护的语句必须使用花括号括起来，不能为单条语句使用try。
5. Throwable重写了toString()方法，从而可以返回一个包含异常描述的字符串。可以使用println()语句显示这个描述。

### 10.5 多条catch子句

1. 由于单块代码可能会引发多个异常，可以指定多条catch子句，每条catch子句捕获不同类型的异常。
2. 当抛出异常时，按顺序检查每条catch语句，并执行类型和异常能够匹配的第一条catch子句。
3. 执行了一条catch语句之后，会忽略其他catch语句，并继续执行try/catch代码块后面的代码。
4. 当使用多条catch语句时，要重点记住异常子类必须位于所有超类之前，因为使用了某个超类的catch语句会捕获这个超类及其所有子类的异常。

### 10.6 嵌套的try语句

1. try语句可以嵌套，也就是说，一条try语句可以位于另外一条try语句中。
2. 每次遇到try语句时，异常的上下文就会被推入到堆栈中。如果内层的try语句没有为特定的异常提供catch处理程序，堆栈就会弹出该try语句，检查下一条try语句的catch处理程序，查看是否匹配异常。这个过程会一直继续下去，直到找到一条匹配的catch语句，或直到检查完所有嵌套的try语句。如果没有找到匹配的catch语句，Java运行时系统会处理异常。
3. 当涉及方法调用时，可能会出现不那么明显的try语句嵌套。例如，可能在一个try代码块中包含了对某个方法的调用，而在该方法内部又有另外一条try语句。对于这种情况，方法内部的try语句仍然被嵌套在调用该方法的外层try代码块中。

### 10.7 throw

1. 异常不仅可以由Java运行时系统抛出，也可以使用`throw`语句显式地抛出。throw的一般形式如下：

   ```java
   throw ThrowableInstance;
   ```

   其中，ThrowableInstance必须是Throwable或其子类类型的对象。基本类型以及非Throwable类都不能用作异常。

2. 可以通过两种方式获得Throwable对象：`在catch子句中使用参数或者使用new运算符创建Throwable对象`。

3. throw语句之后的执行流会立即停止，所有后续语句都不会执行。检查最近的try代码块，查看是否存在和异常类型相匹配的catch语句。如果没有，就检查下一条try语句，这个过程一直重复下去。如果最终没有找到匹配的catch语句，那么默认的异常处理程序会终止程序并输出堆栈踪迹。

### 10.8 throws

1. 如果方法可能引发自身不进行处理的异常，就必须通过在方法声明中提供`throws`子句来完成该任务。

2. throws子句列出了方法可能抛出的异常类型。除了`Error`和`RuntimeException`及其子类类型的异常之外，对于所有其他类型的异常这都是必需的。

3. 包含throws子句的方法声明的一般形式：
...
1. 不管是否有异常抛出，都会执行finally代码块。
2. 如果抛出了异常，那么即使没有catch语句能匹配异常，finally代码块也仍将执行。
3. 只要方法从try/catch代码块内部返回到调用者，不管是通过未捕获的异常还是使用显式的返回语句，都会在方法返回之前执行finally子句。
4. finally子句是可选的。但是，每条try语句至少需要有一条catch子句或finally子句。
...
1. 只需要定义Exception的子类就可以创建异常子类，它不需要实际实现任何内容——只要它们存在于类型系统中，就可以将它们用作异常。

2. 有关Exception类的两个公有构造函数：

   ```java
   Exception() // 创建没有描述的异常
   Exception(String msg) // 为异常指定描述信息
   ```
...
1. 线程的优先级用于决定`何时从一个运行的线程切换到下一个`，这称为上下文切换（context switch）。

#### 11.1.2 同步

1. Java没有提供“Monitor”类；相反，每个对象都有自己的隐式监视器。如果调用对象的同步方法，就会自动进入对象的隐式监视器。一旦某个线程位于一个同步方法中，其他线程就不能调用同一对象的任何其他同步方法。因为语言本身内置了同步支持，所以可以编写出非常清晰且简明的多线程代码。

#### 11.1.3 消息传递

1. Java的消息传递系统允许某个线程进入对象的同步方法，然后进行等待，直到其他线程显式地通知这个线程退出为止。

#### 11.1.4 Thread类和Runnable接口

1. Java的多线程系统是基于Thread类、Thread类的方法及其伴随接口Runnable而构建的。

2. Thread类封装了线程的执行。因为不能直接引用正在运行的线程的细微状态，所以需要通过代理进行处理，Thread实例就是线程的代理。
...
4. Thread类定义了一些用于帮助管理线程的方法，如下图所示：
...
1. 主线程概念
   ...
   > 当Java程序启动时，会立即开始运行一个线程，因为它是程序开始时执行的线程，所以这个线程通常称为程序的主线程。
...
3. 通过Thread类的静态方法currentThread()获取对主线程的一个引用，其一般形式如下所示：
